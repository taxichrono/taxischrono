...
// les reservations d'un chauffeurs en course.
  ...
  static Future<int> sendToChauffeur(idchauffeur, String reservation) async {
    await firestore
        .collection("Reservation")
        .doc(reservation)
        .get()
        .then((value) async {
      final reserv = Reservation.fromJson(value.data()!);
      if (reserv.etatReservation == 0) {
        await firestore
            .collection('Courses')
            .doc(idchauffeur)
            .collection("Request")
            .doc(reserv.idReservation)
            .set(reserv.tomap());
      } else if (reserv.etatReservation == 1) {
        return 1;
      } else {
        return -1;
      }
