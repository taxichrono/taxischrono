...
// Collection variable
  ...
  noterChauffeur(double noteChauffeur) async {
  ...
  static Stream<List<TransactionApp>?> currentTransaction(iduser) => firestore
      ...
      .where('idClient', isEqualTo: iduser)
      .where("etatTransaction", whereIn: [1, 0])
      .snapshots()
      .map((event) {
        return event.docs
...
// ceci permet de vérifier qu'un chauffeur a eccepté la transaction.

  static Future<List<TransactionApp>?> transactioncheck(iduser) => firestore
      .collection("TransactionApp")
      .where('idClient', isEqualTo: iduser)
