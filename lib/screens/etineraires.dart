...
class _SearchDestinaitionPageState extends State<SearchDestinaitionPage> {
  ...
  currentAddess() async {
    ...
    // await GooGleMapServices.requestLocation().then((val) {
    final position = await Geolocator.getCurrentPosition();
    final location = LatLng(position.latitude, position.longitude);

    final start = await GooGleMapServices.getNamePlaceFromPosition(location);
    setState(() {
      startPlace = start;
      controllerstart.text = start!.mainName;
    });
